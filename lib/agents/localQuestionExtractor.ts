...
export function extractQuestionLocally(transcript: string): LocalExtractedQuestion | null {
  ...
  const questionPatterns = [
    ...
    // Question words — immediate verb ("what is X")
    { pattern: /(?:what|how|why|when|where|which|who)\s+(?:is|are|do|does|did|was|were|will|would|could|should)\s+([^.?!]+)/i, confidence: 0.7 },
    // Question words — verb after a noun phrase ("which pooling layer should be applied")
    { pattern: /(?:what|how|why|when|where|which|who)\s+.+?\s+(?:is|are|do|does|did|was|were|will|would|could|should)\s+([^.?!]+)/i, confidence: 0.6 },
    // Question words — catch-all ("which pooling method")
  ...
  for (const { pattern, confidence } of questionPatterns) {
    ...
    if (match && match[1]) {
      // For question-word patterns (what/how/which...), use the full match
      // so "Which pooling layer should be applied..." isn't truncated to just "applied..."
      const isQuestionWordPattern = /^\(\?:what\|how\|why/.test(pattern.source);
      const questionText = (isQuestionWordPattern ? match[0] : match[1]).trim();
